...
public class Arena : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        int count = 0;
        float offset = 0f;
        for (int i = 0; i < (m_numberOfCharacters < m_gameManager.CharacterList.Count ? m_numberOfCharacters : m_gameManager.CharacterList.Count); i++)
        {
            m_characters.Add(m_gameManager.CharacterList[i]);
            m_characters[i].transform.position = ((m_distance * 0.5f * (m_collums-1)) - (count * m_distance)) * transform.forward + offset * transform.right;
            count++;
